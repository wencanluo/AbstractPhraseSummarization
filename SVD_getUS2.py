...
def getSVD(prefix, np, K, corpusname="corpus", ngrams=[1,2]): 
    ...
    cscname = path + "_".join(types) + '_' + corpusname + cscexe
    #print cscname
    #SaveCSC(scipy_csc_matrix, cscname)
#     for vector in scipy_csc_matrix:
#         print vector
    
    #http://docs.scipy.org/doc/scipy-0.14.0/reference/generated/scipy.sparse.linalg.svds.html
    u, s, vt = sparsesvd(scipy_csc_matrix.T, K) # do SVD, asking for 100 factors
    
    print type(u), u.shape
    #save_sparse_csr(prefix + "u" + cscexe, u)
    #u.tofile(prefix + "u" + cscexe)
    numpy.savetxt(path + "u" + '_'+ str(K) + '_' + corpusname + cscexe, u)
    
    print s.shape
    #save_sparse_csr(prefix + "s" + cscexe, s)
    #s.tofile(prefix + "s" + cscexe)
    numpy.savetxt(path + "s" + '_'+ str(K) + '_' + corpusname+cscexe, s)
    
    print vt.shape
    #save_sparse_csr(prefix + "vt" + cscexe, vt)
    #vt.tofile(prefix + "vt" + cscexe)
    numpy.savetxt(path + "vt" + '_'+ str(K) + '_' + corpusname+cscexe, vt)
    
    #save the new matrix
    newA = numpy.dot(u, numpy.dot(numpy.diag(s), vt))
    
    newAname = path + "_".join(types) + '_' + str(K) + '_' + corpusname + cscexe
