...
def getAverageWordLength(excelfile):
    ...
    for i, sheet in enumerate(sheets):
        ...
        for type in ['POI', 'MP', 'LP']:
            student_responseList = getStudentResponseList(orig, header, summarykey, type, withSource=False, Split=False)
...
def GetTable1forENMLP(excelfile):
    ...
    for i, sheet in enumerate(sheets):
        ...
        for type in ['POI', 'MP', 'LP']:
            student_responseList = getStudentResponseList(orig, header, summarykey, type, withSource=False, Split=True)
