...
class Decoder(object):
    ...
    def decode(self, instance, oracle_len='nolen', node_cost=None, edge_cost=None):
        ...
        print '[num_nonnegative_edges]: %d' % num_nonnegative_edges
        ...
        print '[num_gold_nodes]: %d' % num_gold_nodes
        print '[num_selected_nodes]: %d' % len(selected_nodes)
        print '[num_gold_edges]: %d' % num_gold_edges
        print '[num_selected_edges]: %d' % len(selected_edges)
...
#             self.model.addConstr(gp.LinExpr(dummyEdgeFlowVars[i] * num_nodes - self.dummyEdgeVars[i,1]) >= 0.0)
    ...
    def decode(self, node_weights, edge_weights, root_nodes, 
        ...
        print 'adding ILP variables...'
        self.addGraphVars()
        self.addDummyEdges(root_nodes)
        
        print 'adding ILP graph constraints...'
        self.addGraphConstrs()
        
        print 'adding ILP flow constraints...'
        self.addFlowConstrs()
        
        print 'adding ILP other constraints...'
        self.addLenConstrs()
        self.addIncomingEdgeConstrs()
        self.addExclusionConstrs(edge_weights) # edge_weights changed here

        # set objective
        print 'adding ILP objective...'
